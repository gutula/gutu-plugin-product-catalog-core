export type ProductCatalogCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildProductCatalogCoreSqliteMigrationSql(options: ProductCatalogCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "product_catalog_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getProductCatalogCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getProductCatalogCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildProductCatalogCoreSqliteRollbackSql(options: ProductCatalogCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "product_catalog_core_");
  return [
...
export function getProductCatalogCoreSqliteLookupIndexName(tablePrefix = "product_catalog_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getProductCatalogCoreSqliteStatusIndexName(tablePrefix = "product_catalog_core_"): string {
