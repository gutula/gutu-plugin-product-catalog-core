export type ProductCatalogCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildProductCatalogCoreMigrationSql(options: ProductCatalogCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "product_catalog_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getProductCatalogCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getProductCatalogCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildProductCatalogCoreRollbackSql(options: ProductCatalogCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "product_catalog_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "product_catalog_core";
  return [
...
export function getProductCatalogCoreLookupIndexName(): string {
  return "product_catalog_core_primary_lookup_idx";
}

export function getProductCatalogCoreStatusIndexName(): string {
  return "product_catalog_core_exception_status_idx";
}
